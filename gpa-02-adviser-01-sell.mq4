//+------------------------------------------------------------------+
//|                                                       adv-sell.mq4 |
//| Expert Advisor that sells a stock/symbol
//+------------------------------------------------------------------+
...
/* This Expert Adviser the last 10 bars (only once, in the OnInit() function) and then sends a Sell order (only once).
...
int OnInit()
{
   ...
   const float VOLUME_TO_SELL = 0.01;
   ...
   // VOLUME_TO_SELL: how many stocks/units to sell
   // Bid: the selling price (built-in global variable, does not need to be declared)
   orderTicket = OrderSend(Symbol(), OP_SELL, VOLUME_TO_SELL, Bid, 0, 0, 0);
